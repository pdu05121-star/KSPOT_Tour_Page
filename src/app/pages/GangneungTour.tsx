...
//
// ★ 2026-07-29(2차) — 출발 시각 조정 + 스팟별 체류시간 명시 반영 ★
// 팀 피드백: 첫차(05:06)는 실제 이용자 기준 너무 이름 → 좀 더 현실적인 아침 시간대로 조정.
// 출발: 서울역 07:59 KTX-이음 805(실제 시간표, 출처: train.asamaru.net 2026-07-29 조회) → 강릉역 10:04 도착.
// 귀환: 강릉역 20:30 KTX-이음 824 → 서울역 22:32 도착(824를 놓치면 826편 21:33→23:26이 대안).
// 아래 TIMETABLE은 각 스팟의 "권장 체류시간"(스팟별로 30분~1시간, 코스템플릿 문서 기준)에 스팟 간
// 이동시간 추정치(같은 권역 내 도보/짧은 이동은 10~15분, 권역 이동은 20~40분으로 추정 — 전부 미확정 추정치)를
// 더해 순차적으로 계산한 것입니다. 10:04 도착 기준으로 계산하면 마지막 스팟(보사노바) 종료가 약 18:10경
// → 강릉역 도착 약 19:30 → 824편(20:30) 탑승까지 약 1시간 버퍼. 05:06 출발안(버퍼 약 1시간30분)보다는
// 약간 빠듯하지만, 정상적인 환승 여유로는 충분하다고 판단. 더 넉넉하게 가려면 826편(21:33)을 대안으로 안내.
// 이동시간 추정치가 전부 미확정이라 위 계산도 잠정치이며, 현지 이동수단·정확한 소요시간이 팀 실측으로
// 확인되기 전까지 ROUND_TRIP_CONFIRMED을 true로 바꾸지 않습니다(절대규칙 1번).
//
// ★ 2026-07-29(4차) — 대중교통 전구간 실측 완료 + 코스 재설계 ★
// 기존 11개 스팟 일정(이동시간 합계 약 18h54m)은 당일치기 가능시간(약 10.5h)을 크게 초과.
// 팀 결정: 스팟 수를 줄인다 → 주문진 3개 성지(BTS·소돌·방파제) + 안목 2개(강문해변·보사노바).
// 동선 효율화: 북쪽 BTS(최북단)→소돌→방파제(최남단) 순으로 단방향 이동, 역방향 제거.
// 실측 이동시간(Google Maps 대중교통, 2026-07-29):
//   강릉역→BTS 1h40m / BTS→소돌 45m / 소돌→방파제 50m / 방파제→강릉역 귀환 1h30m
//   강릉역→강문해변 ~30m(추정, 미실측) — 팀 현장 확인 필요.
// ⚠️ 남은 미확정: 강릉역↔강문해변 정확한 시간, 주문진항 물회집 팀 선정.
//
...
// 출발/귀환 허브 — 팀 결정: 서울역 (KTX-이음 기준). 출발 07:59 / 귀환 20:30 확정(위 검증 참고).
...
const SPOTS: SpotItem[] = [
  {
    no: "01", emoji: "💜", tag: "K-POP 스팟", title: "BTS 버스정류장",
    ...
    tip: "정류장 표지판과 보라색 벤치를 함께 프레임에 담고, 인적 드문 이른 아침에 방문하면 줄 서지 않고 찍을 수 있어요.",
    caution: "실제 운행하는 버스정류장이 아니라 포토존 세트이니, 주변 도로 통행에 주의해 주세요.",
    image: gangneungPlaceholderImg,
  },
  {
    no: "02", emoji: "🌙", tag: "OTT 드라마 스팟", title: "소돌 방파제",
    subtitle: "넷플릭스 〈더 글로리〉 16화 촬영지",
  ...
  },
  {
    ...
    coord: "강원 강릉시 주문진읍 해안로 1609 (영진해변 방파제)",
...
const TIMETABLE: TimetableItem[] = [
  { time: "07:59", emoji: "🚄", label: `${HUB_STATION} 출발`, desc: "KTX-이음 805 탑승 (실제 시간표 기준)" },
  { time: "10:04", emoji: "🚄", label: "강릉역 도착", desc: "KTX 하차 → 버스로 BTS 버스정류장(향호리) 방면 이동 (약 1시간 40분, 실측)" },
  { time: "11:44", emoji: "💜", label: "BTS 버스정류장", desc: "보라색 벤치·포토존 그네에서 사진 촬영 (체류 약 20분)" },
  { time: "12:04", emoji: "🚌", label: "소돌 방파제로 이동", desc: "버스 약 45분 (실측)" },
  { time: "12:49", emoji: "🌙", label: "소돌 방파제", desc: "더 글로리 빨간 등대 배경 사진 (체류 약 20분)" },
  { time: "13:09", emoji: "🚌", label: "주문진 방파제로 이동", desc: "버스 약 50분 (실측)" },
  { time: "13:59", emoji: "🌊", label: "주문진 방파제", desc: "도깨비 첫 만남 장소에서 사진 촬영 (체류 약 30분)" },
  { time: "14:29", emoji: "🐟", label: "주문진항 점심", desc: "방파제 바로 옆 항구에서 물회 점심 (체류 약 1시간, 식당은 팀 선정 예정)" },
  { time: "15:35", emoji: "🚌", label: "강릉역으로 귀환", desc: "버스 약 1시간 30분 (실측)" },
  { time: "17:05", emoji: "🚄", label: "강릉역 도착", desc: "잠시 휴식 후 강문해변으로 이동" },
  { time: "17:35", emoji: "🌅", label: "강문해변", desc: "그녀는 예뻤다 촬영지, 솟대다리 산책 (체류 약 20분, 이동시간 팀 확인 필요)" },
  { time: "17:55", emoji: "☕", label: "보사노바 커피로스터스", desc: "강문해변에서 도보 이동, 오션뷰 커피로 하루 마무리 (체류 약 1시간)" },
  { time: "19:05", emoji: "🚌", label: "강릉역으로 이동", desc: "버스 약 30분 (추정, 팀 확인 필요)" },
  { time: "19:35", emoji: "🚄", label: "강릉역 도착", desc: "KTX 탑승 전 약 55분 여유" },
    ...
    >
        ...
        >
          ...
          {!ROUND_TRIP_CONFIRMED && (
            <div className="px-4 py-2.5 text-white" style={{ backgroundColor: WARN_AMBER }}>
              ...
              >
                {HUB_STATION} 07:59 출발 → 강릉역 20:30 출발/22:32 도착(KTX-이음 실제 시간표 기준)으로 당일치기 자체는 가능한 것으로 확인했어요. 대중교통 전구간 실측 완료(강릉역↔BTS·소돌·방파제), 강릉역↔강문해변 구간과 주문진항 식당 팀 선정이 남아있어 GO 판정은 아직입니다.
              </div>
      ...
      <section className="max-w-2xl mx-auto px-5 sm:px-8 mt-20 sm:mt-24">
        ...
        <p className="text-[11px] mb-6" style={{ color: INK, opacity: 0.55 }}>
          ※ KTX 구간과 강릉역↔주문진(BTS·소돌·방파제) 구간은 실제 코레일 시간표·Google Maps 대중교통 기준으로 실측한 값입니다. 강릉역↔강문해변 구간(~30분)은 추정치이며 팀 현장 확인이 필요합니다. 주문진항 물회 식당은 팀 선정 예정입니다.
        </p>
