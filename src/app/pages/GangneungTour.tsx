...
// 출발/귀환 허브 — 팀 결정: 서울역 (KTX-이음 기준). 출발 08:57(807편) / 귀환 21:33(826편) 확정(위 검증 참고).
...
const TIMETABLE: TimetableItem[] = [
  ...
  { time: "10:58", emoji: "🚄", label: "강릉역 도착", desc: "KTX 하차 → 버스로 BTS 버스정류장(향호리) 방면 이동 (약 1시간 40분, 실측)" },
  { time: "12:38", emoji: "💜", label: "BTS 버스정류장", desc: "보라색 벤치·포토존 그네에서 사진 촬영 (체류 약 20분)" },
  { time: "12:58", emoji: "🚌", label: "소돌 방파제로 이동", desc: "버스 약 45분 (실측)" },
  { time: "13:43", emoji: "🌙", label: "소돌 방파제", desc: "더 글로리 빨간 등대 배경 사진 (체류 약 20분)" },
  { time: "14:03", emoji: "🚌", label: "주문진 방파제로 이동", desc: "버스 약 50분 (실측)" },
  { time: "14:53", emoji: "🌊", label: "주문진 방파제", desc: "도깨비 첫 만남 장소에서 사진 촬영 (체류 약 30분)" },
  { time: "15:23", emoji: "🐟", label: "주문진항 점심", desc: "방파제 바로 옆 항구에서 물회 점심 (체류 약 1시간, 식당은 팀 선정 예정)" },
  { time: "16:23", emoji: "🚌", label: "강릉역으로 귀환", desc: "버스 약 1시간 30분 (실측)" },
  { time: "17:53", emoji: "🚄", label: "강릉역 도착", desc: "잠시 휴식 후 강문해변으로 이동" },
  { time: "18:23", emoji: "🌅", label: "강문해변", desc: "그녀는 예뻤다 촬영지, 솟대다리 산책 (체류 약 20분, 이동시간 팀 확인 필요)" },
  { time: "18:43", emoji: "☕", label: "보사노바 커피로스터스", desc: "강문해변에서 도보 이동, 오션뷰 커피로 하루 마무리 (체류 약 1시간)" },
  { time: "19:43", emoji: "🚌", label: "강릉역으로 이동", desc: "버스 약 30분 (추정, 팀 확인 필요)" },
  { time: "20:13", emoji: "🚄", label: "강릉역 도착", desc: "KTX 탑승 전 약 1시간 20분 여유" },
  { time: "21:33", emoji: "🚄", label: `${HUB_STATION} 방향 출발`, desc: "KTX-이음 826 탑승 (실제 시간표 기준)" },
  { time: "23:26", emoji: "🏠", label: `${HUB_STATION} 도착`, desc: "귀환" },
];
        ...
        >
          ...
          {!ROUND_TRIP_CONFIRMED && (
            <div className="px-4 py-2.5 text-white" style={{ backgroundColor: WARN_AMBER }}>
              ...
              >
                {HUB_STATION} 08:57 출발 → 강릉역 21:33 출발/23:26 도착(KTX-이음 실제 시간표 기준)으로 당일치기 자체는 가능한 것으로 확인했어요. 대중교통 전구간 실측 완료(강릉역↔BTS·소돌·방파제), 강릉역↔강문해변 구간과 주문진항 식당 팀 선정이 남아있어 GO 판정은 아직입니다.
              </div>
