...
// ⚠️ 남은 미확정: 강릉역↔강문해변 이동시간(~30분 추정), 주문진항 물회집 팀 선정.
//
...
const TIMETABLE: TimetableItem[] = [
  ...
  { time: "13:34", emoji: "🚌", label: "강릉역으로 귀환", desc: "300번 버스 약 1시간 01분 (실측)" },
  { time: "14:35", emoji: "🚄", label: "강릉역 도착", desc: "잠시 휴식 후 강문해변으로 이동" },
  { time: "15:05", emoji: "🌅", label: "강문해변", desc: "그녀는 예뻤다 촬영지, 솟대다리 산책 (체류 약 20분, 이동시간 팀 확인 필요)" },
  { time: "15:25", emoji: "☕", label: "보사노바 커피로스터스", desc: "강문해변에서 도보 이동, 오션뷰 커피로 하루 마무리 (체류 약 1시간)" },
  { time: "16:25", emoji: "🚌", label: "강릉역으로 이동", desc: "버스 약 30분 (추정, 팀 확인 필요)" },
  { time: "16:55", emoji: "🚄", label: "강릉역 도착", desc: "KTX 탑승 전 약 1시간 53분 여유" },
    ...
    >
      ...
      <section className="max-w-2xl mx-auto px-5 sm:px-8 mt-20 sm:mt-24">
        ...
        <p className="text-[11px] mb-6" style={{ color: INK, opacity: 0.55 }}>
          ※ KTX 구간과 강릉역↔주문진(도깨비·더글로리·BTS) 전 구간은 팀 현장 실측값입니다. 시티1 구간(도깨비→더글로리, 8분)은 40분 배차이므로 현장 시간표 확인 필수. 강릉역↔강문해변(~30분)은 추정치, 팀 확인 필요. 주문진항 물회 식당은 팀 선정 예정입니다.
        </p>
