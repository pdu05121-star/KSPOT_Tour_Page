...
// BTS→강문해변: 시티1 직행 확인(실측) — 주문진해변발 탑승, 강문해변 정류장 37분 후 하차, 도보 5분.
// 주문진항 점심 추가: 도깨비(12:18) → 시티1(12:22 통과) → 주문진항(12:27) → 1h 식사 → 13:27
...
const EATS: EatItem[] = [
  {
    ...
    tip: "주문진항은 동해안 최대 어항 중 하나로, 물회 전문점이 모여 있어요. 도깨비촬영지 관람 직후 시티1로 5분 거리.",
    view: "도깨비촬영지 바로 다음 코스. 동해 활어를 바로 앞에서 맛볼 수 있어요.",
    image: gangneungPlaceholderImg,
  },
  {
    section: "food", emoji: "🏪", category: "귀환 전 · 시장", title: "강릉중앙시장 (팀 선정 예정)",
    coord: "강릉시 성남동 일대 (강릉역 도보 약 15분)",
    tip: "귀환 KTX 탑승 전 마지막 코스. 오징어순대·명태회·메밀부침 등 강릉 고유 먹거리를 맛볼 수 있어요.",
    view: "강릉 현지인들이 즐겨 찾는 전통시장. 강릉역에서 도보로 이동 가능.",
    image: gangneungPlaceholderImg,
  },
  {
...
const TIMETABLE: TimetableItem[] = [
  ...
  { time: "11:58", emoji: "🌊", label: "도깨비 촬영지", desc: "교항리 방파제, 도깨비 첫 만남 장소에서 사진 촬영 (체류 약 20분)" },
  { time: "12:18", emoji: "🚌", label: "주문진항으로 이동", desc: "시티1(12:22 도깨비 통과, 안목발 6회) → 주문진항 하차 약 5분" },
  { time: "12:27", emoji: "🐟", label: "주문진항 점심", desc: "동해 활어 물회 (체류 약 1시간, 식당 팀 선정 예정)" },
  { time: "13:27", emoji: "⏳", label: "시티1 대기", desc: "주문진항 13:57 통과 편(안목발 8회, 13:20 출발) 탑승 — 약 30분 대기 ⚠️ 40분 배차" },
  { time: "13:57", emoji: "🚌", label: "시티1 → 더글로리촬영지", desc: "주문진항에서 약 3분" },
  { time: "14:00", emoji: "🌙", label: "더글로리 촬영지", desc: "소돌해변 빨간 등대 배경 사진 (체류 약 20분)" },
  { time: "14:20", emoji: "🚶", label: "BTS로 도보 이동", desc: "해변가를 따라 도보 약 24분 (실측)" },
  { time: "14:44", emoji: "💜", label: "BTS 버스정류장", desc: "보라색 벤치·포토존 그네에서 사진 촬영 (체류 약 20분)" },
  { time: "15:04", emoji: "🚶", label: "주문진해변 정류장으로 이동", desc: "BTS에서 도보 6분 (실측)" },
  { time: "15:10", emoji: "🚌", label: "시티1 → 강문해변 직행", desc: "주문진해변발 10회(15:10 출발), 강문해변 정류장까지 37분 (실측)" },
  { time: "15:52", emoji: "🌅", label: "강문해변", desc: "그녀는 예뻤다 촬영지, 솟대다리 산책 (정류장 하차 후 도보 5분, 체류 약 20분)" },
  { time: "16:12", emoji: "☕", label: "보사노바 커피로스터스", desc: "안목해변 대표 카페, 오션뷰 커피로 하루 마무리 (체류 약 1시간)" },
  { time: "17:12", emoji: "🚌", label: "강릉역으로 이동", desc: "버스 약 30분 (추정, 팀 확인 필요)" },
  { time: "17:42", emoji: "🏪", label: "강릉중앙시장", desc: "강릉역에서 도보 약 15분. 오징어순대·명태회 등 강릉 로컬 먹거리 (체류 약 45분, 식당 팀 선정 예정)" },
  { time: "18:27", emoji: "🚶", label: "강릉역으로 이동", desc: "시장에서 도보 약 15분" },
  { time: "18:42", emoji: "🚄", label: "강릉역 도착", desc: "KTX 탑승 전 약 1시간 48분 여유" },
